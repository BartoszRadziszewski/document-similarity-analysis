...
# ==============================
# CONFIGURATION AND SETUP - FIXED for cross-platform
# ==============================
...
class ClusterExampleExtractor:
    ...
    def __init__(self, project_name: str, base_dir: str = None):
        ...
        self.clusters_folder = os.path.join(self.project_dir, "clusters_v3")
    ...
    def _load_cluster_metadata(self) -> Dict:
        """Load cluster metadata from existing analysis files"""
        metadata = {}
        ...
        if os.path.exists(self.summary_v3_file):
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    metadata[cluster_id] = {
                        'type': row.get('Type', 'Content'),
                        'summary': row.get('Summary', ''),
                        'paragraph_count': row.get('Paragraph_Count', 0)
                    }
            except Exception as e:
                self.logger.warning(f"Could not load cluster summaries: {e}")
            ...
            try:
                ...
                for cluster_id in df['Cluster_Label'].unique():
                    ...
                    if cluster_id not in metadata:
                        metadata[cluster_id] = {}
                    metadata[cluster_id].update({
                        'paragraph_count': len(cluster_data),
                        'avg_similarity': cluster_data.get('Avg_Similarity_In_Cluster', [0]).mean() if 'Avg_Similarity_In_Cluster' in cluster_data.columns else 0
                    })
            except Exception as e:
    ...
    def _get_sampling_strategy(self, paragraphs: List[str], n_examples: int, strategy: str) -> List[str]:
        ...
        if strategy == 'balanced':
            ...
            if n_examples >= 2:
                examples = [paragraphs[0], paragraphs[-1]]
                remaining = n_examples - 2
            ...
            # Spread examples across the entire cluster
            indices = []
            step = len(paragraphs) / n_examples
            for i in range(n_examples):
                idx = int(i * step)
                indices.append(min(idx, len(paragraphs) - 1))
            examples = [paragraphs[i] for i in sorted(set(indices))]
            
        elif strategy == 'representative':
            # Random sampling but ensure first and last are included
            examples = [paragraphs[0]]
            if n_examples > 1:
    ...
    def should_include_cluster(self, cluster_name: str, exclude_types: List[str], min_size: int) -> bool:
        ...
        if cluster_name in self.cluster_metadata:
            cluster_info = self.cluster_metadata[cluster_name]
            
            # Check type exclusion
            cluster_type = cluster_info.get('type', 'Content')
            if cluster_type in exclude_types:
                return False
                
            # Check minimum size
            cluster_size = cluster_info.get('paragraph_count', 0)
        ...
        for item in data:
            ...
            if cluster != current_cluster:
                ...
                if cluster in self.cluster_metadata:
                    ...
                    meta_para.add_run(f"{metadata.get('type', 'N/A')} | ")
                    meta_para.add_run("Paragraph Count: ").bold = True
                    meta_para.add_run(f"{metadata.get('paragraph_count', 'N/A')} | ")
                    meta_para.add_run("Average Similarity: ").bold = True
                    meta_para.add_run(f"{metadata.get('avg_similarity', 0):.3f}")
                    
                    if 'summary' in metadata and metadata['summary']:
                        summary_para = doc.add_paragraph()
                        summary_para.add_run("Summary: ").bold = True
                        summary_para.add_run(metadata['summary'])
    ...
    def run(self, n_examples: int = 10, strategy: str = 'balanced', 
        ...
        # Create outputs based on format selection
